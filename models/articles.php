...
public static function create($title, $abstract, $text, $userId){
        $db = Db::getInstance();
        ...
        $query = "INSERT INTO articles(title, abstract, text, user_id) VALUES ('$title', '$abstract', '$text', '$userId');";
        if($db->query($query)){
            return true;
        }
        else{
    ...
    public static function update($id, $title, $abstract, $text, $userId){
        $db = Db::getInstance();
        $id = intval($id);
        $title = mysqli_real_escape_string($db, $title);
        $abstract = mysqli_real_escape_string($db, $abstract);
        ...
        $userId = intval($userId);
         ...
         $query = "UPDATE articles SET title='$title', abstract='$abstract', text='$text' WHERE id='$id' AND user_id='$userId'";
        if($db->query($query)){
            return true;
        }
        else{
    ...
    public static function delete($id, $userId){
        ...
        $query = "DELETE FROM articles WHERE id='$id' AND user_id='$userId'";
        if ($db->query($query)) {
            return true;
        } else {
            echo mysqli_error($db);
...
public static function findMine($user_id)
    {
        $db = Db::getInstance();
